...
public class BaselineVsPpebsComparison {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("╔════════════════════════════════════════════════════════════════════╗");
        if (runAll) {
            System.out.println("║   WaltzDB Benchmark: 3-Way Comparison (All Strategies)              ║");
        } else {
        ...
        if (runAll) {
            System.out.println("Mode: Comparing Baseline vs BASE_POINT vs LABEL_PROPAGATION");
        }
        System.out.println();
        ...
        Map<String, Map<Integer, PpebsStats>> allPpebsResults = new LinkedHashMap<>();

        // Determine which strategies to run
        WaltzDbPhasedParallelBenchmark.PartitionStrategy[] strategies;
        if (runAll) {
            strategies = new WaltzDbPhasedParallelBenchmark.PartitionStrategy[] {
                    WaltzDbPhasedParallelBenchmark.PartitionStrategy.BASE_POINT,
                    WaltzDbPhasedParallelBenchmark.PartitionStrategy.LABEL_PROPAGATION
            };
        } else {
            strategies = new WaltzDbPhasedParallelBenchmark.PartitionStrategy[] {
                    WaltzDbPhasedParallelBenchmark.PartitionStrategy.BASE_POINT
        ...
        for (WaltzDbPhasedParallelBenchmark.PartitionStrategy strategy : strategies) {
            Map<Integer, PpebsStats> ppebsResults = new LinkedHashMap<>();
            System.out.println("\n>>> Running PPESB with " + strategy + " partitioning...");
            ...
            for (int threads : threadCounts) {
                ...
                for (int i = 0; i < MEASURED_ITERATIONS; i++) {
                    WaltzDbPhasedParallelBenchmark benchmark = new WaltzDbPhasedParallelBenchmark(
                            DATA_FILE, threads, false, strategy);
                ...
                System.out.printf("        Average: %d ms (min=%d, max=%d)\n",
                        stats.avgTotal, stats.minTotal, stats.maxTotal);
            }
            allPpebsResults.put(strategy.name(), ppebsResults);
        }
        ...
        // Baseline
        System.out.printf("%-35s │ %-12s │ %-12s │ %-10d │ %-12s\n",
                "Original Drools",
                baselineAvgTime + " ms",
        ...
        // PPESB variants for each strategy
        for (Map.Entry<String, Map<Integer, PpebsStats>> strategyEntry : allPpebsResults.entrySet()) {
            String strategyName = strategyEntry.getKey();
            Map<Integer, PpebsStats> ppebsResults = strategyEntry.getValue();

            for (int threads : threadCounts) {
                PpebsStats stats = ppebsResults.get(threads);
                double speedup = baselineAvgTime / (double) stats.avgTotal;
                System.out.printf("%-35s │ %-12s │ %-12s │ %-10d │ %-12s\n",
                        "PPESB-" + strategyName + " (" + threads + "t)",
                        stats.avgTotal + " ms",
        ...
        long maxDiff = 0;
        for (Map<Integer, PpebsStats> ppebsResults : allPpebsResults.values()) {
